...
It's also an experiment in how investing can work now. The entire "wealth management firm" runs **inside [Claude Code](https://claude.com/claude-code)**, with no backend, no API keys, and no app. Just plain local files as the single source of truth, an AI advisory team that researches, audits, and challenges every decision, and a self-contained dashboard that regenerates after every cycle.
...
The routine: **₱5,000/week parked, invested once a month** (about ₱17.6k into VT, about ₱2.4k into alternating REIT board lots), every dividend reinvested, no timing, no chasing, never sell the core. At graduation (~late 2027): contributions rise, VT migrates to VWRA via IBKR for tax efficiency, and the whole strategy gets re-run against a real salary.
...
| <img src="assets/agents/sentinel.svg" width="44"> | **SENTINEL** · Risk | Scores the portfolio 1 to 10 on concentration, volatility, currency exposure, and the higher-risk satellite, then compares it to last cycle. |
...
| <img src="assets/agents/translator.svg" width="44"> | **TRANSLATOR** · Analyst | Turns research into 2 or 3 options for *this* portfolio (always including "do nothing"), with pros, cons, and a stated confidence. Never commands. |
...
| <img src="assets/agents/desk.svg" width="44"> | **THE DESK** · Summary | Merges it all into one dated digest (headline, changes, flags, conflicts side by side), then regenerates the dashboard. |
...
- **Income**: projected quarterly REIT income and the dividend log. The "is this working?" page, where every payout that lands and gets reinvested is the flywheel turning.
...
The files holding my actual personal and financial data (`profile.json`, `holdings.json`, `account_reference.json`) stay local and are gitignored, so they never get committed. The repo ships sanitized `*.example.json` templates in their place; copy one, drop the `.example`, and fill in your own numbers.
